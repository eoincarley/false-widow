...
broad range of outdoor and indoor environments at temperate latitudes far beyond its native range \citep{Nentwig2015}.
...
Monitoring the geospatial distribution of \textit{S.~nobilis} has been aided by the rapid growth of citizen-science platforms 
such as iNaturalist and the Global Biodiversity Information Facility (GBIF). These platforms have greatly expanded the volume of 
georeferenced occurrence data available for invasive species research \citep{Feldman2021}. 
For \textit{S.~nobilis}, the platforms now provide over 10,000 research-grade records spanning Western Europe, 
offering an unprecedented opportunity to model the species' environmental niche at continental scale 
--- provided that spatial biases in recording effort are carefully accounted for \citep{Beck2014}.

In terms of the environmental conditions where \textit{Steatoda nobilis} can be found, it is strongly synanthropic, 
meaning it is preferentially associated with human-made structures. 
In temperate regions, it is most frequently encountered on the exterior walls of heated buildings, under window sills, and 
around artificial lighting that attracts nocturnal prey \citep{Dugon2017, Bauer2019}. This ecological dependence on the built 
environment suggests that remotely sensed indicators of urbanisation may serve as effective predictors of the species' distribution.
 Artificial light at night (ALAN), which has increased in both extent and radiance globally at approximately 2\% per year 
 \citep{Kyba2017}, is now recognised as a major anthropogenic driver of ecological change, affecting the behaviour, physiology, 
 and demography of a wide range of taxa \citep{Gaston2015, GastonBennie2014, Holker2010}. Similarly, land surface temperature 
 (LST) measured by satellite sensors captures the urban heat island effect --- the phenomenon whereby built-up areas are warmer 
 than surrounding rural land --- which can facilitate the establishment of non-native ectotherms at otherwise inhospitable
  latitudes \citep{Meineke2013}.

Satellite remote sensing now provides provides a means of monitoring 
spatially continuous measurements of both ALAN and LST at continental scales. 
The Visible Infrared Imaging Radiometer Suite (VIIRS) Day/Night Band produces monthly composites of nighttime radiance 
that serve as a direct proxy for artificial lighting intensity \citep{Elvidge2017, Falchi2016}. 
The Moderate Resolution Imaging Spectroradiometer (MODIS) aboard the Terra and Aqua satellites provides daytime and 
nighttime LST at 1~km resolution \citep{Wan2014}, while the Landsat 8 and 9 Thermal Infrared Sensors offer LST at a 
finer 30~m native resolution, enabling more detailed characterisation of thermal environments at local scales. 
These satellite-derived products have been increasingly adopted as environmental predictors in ecological 
and biodiversity research \citep{Pettorelli2014, Waltari2014}, but their application to invasive species 
distribution modelling remains limited.

Overall, the availability of georeferenced occurrence data and satellite monitoring of ALAN and LST now provide a means of 
Species distribution models (SDMs): they relate known occurrence localities 
to environmental conditions to estimate habitat suitability across unsampled areas \citep{Elith2006}. 
Among SDM algorithms, Maximum Entropy (MaxEnt) modelling has become one of the most widely used for presence-only data, 
owing to its strong predictive performance and ability to fit complex, non-linear response functions without requiring 
confirmed absences \citep{Phillips2006, Phillips2008, Elith2011, Merow2013}. However, the choice of model evaluation 
strategy is critical: standard random cross-validation tends to inflate performance 
metrics when occurrence data are spatially autocorrelated, and spatially blocked partitioning schemes are now recommended 
as standard practice \citep{Roberts2017, Valavi2019}.

Despite the growing body of ecological literature on \textit{S.~nobilis}, to our knowledge no previous study has combined satellite-derived 
ALAN and multi-sensor LST data in a species distribution model for this species. Here we address this gap 
by building a MaxEnt model for \textit{S.~nobilis} across Western Europe using four satellite-derived environmental predictors 
--- VIIRS nighttime radiance, MODIS daytime and nighttime LST, and Landsat summer LST --- 
trained on over 10,000 citizen-science occurrence records from iNaturalist and GBIF. 
We evaluate model performance using spatially blocked cross-validation and assess variable contributions through 
permutation importance analysis. We determine whether freely available satellite observations of artificial light and thermal environment can predict 
the distribution of a synanthropic invasive species. This provides a practical framework for prioritising surveillance in 
areas of likely future establishment.
...
\begin{equation}
\mathbf{z} = \begin{pmatrix} z_1 \\ z_2 \\ z_3 \\ z_4 \end{pmatrix} = {\small\begin{pmatrix} \text{VIIRS nighttime radiance (nW\,cm}^{-2}\,\text{sr}^{-1}\text{)} \\ \text{MODIS daytime LST (\textdegree C)} \\ \text{MODIS nighttime LST (\textdegree C)} \\ \text{Landsat summer LST (\textdegree C)} \end{pmatrix}}
\label{eq:zvec}
\end{equation}

MaxEnt does not model these raw variables directly. Instead, it constructs a vector of \emph{derived features} $\mathbf{h}(\mathbf{z})$ that permit non-linear and interactive responses. With our configuration (linear, hinge, and product features across four variables) the feature vector comprises $4 + 40 + 6 = 50$ elements:
\begin{multline}
...
Importantly, MaxEnt is a \textit{presence--background} method: rather than requiring confirmed absences, it contrasts the environmental conditions at known occurrence sites against those sampled from the landscape as a whole (represented by the 10,000 bias-corrected background points described above). The optimisation problem is to find the weight vector $\boldsymbol{\lambda}$ that maximises the regularised log-likelihood:
\begin{equation}
...
\label{eq:loglik}
\end{equation}
where $\mathbf{z}_1, \ldots, \mathbf{z}_n$ are the $n = 9{,}967$ presence locations and $\tilde{\mathbf{z}}_1, \ldots, \tilde{\mathbf{z}}_m$ are the $m = 8{,}343$ background points. The first term rewards high predicted values at presence sites; the second penalises high predictions across the background (preventing the model from predicting high suitability everywhere); the third is an L1 regularisation penalty scaled by a beta multiplier $\beta = 1.5$, which shrinks uninformative feature weights toward zero and produces smoother response curves \citep{Merow2013}.

The maximum-entropy principle requires that, at the optimum, the model-weighted mean of each feature across the background equals the mean of that feature at the presence sites. Because the 50 derived features are deterministic transformations of the raw variables (with hinge knots fixed at predetermined quantiles), their values at every site are known before fitting begins. The target for each feature $h_i$ is its presence-site mean:
\begin{equation}
...
\label{eq:constraint}
\end{equation}
The left-hand side is fixed; the right-hand side is the model-weighted mean across the background, which changes as $\boldsymbol{\lambda}$ is updated. The optimisation adjusts $\boldsymbol{\lambda}$ until the two sides agree for all 50 features simultaneously (up to the regularisation penalty, which relaxes the matching for weakly informative features). This is solved iteratively using the L-BFGS-B quasi-Newton algorithm \citep{Byrd1995}, starting from $\boldsymbol{\lambda} = \mathbf{0}$ (a uniform distribution). Because $\mathcal{L}$ is concave in $\boldsymbol{\lambda}$, the optimum is unique and convergence to the global maximum is guaranteed.
...
Two distinct modelling steps were performed. First, a single MaxEnt model was trained on the \emph{entire} presence--background dataset (9,967 presences and 8,343 background points); this is the model used for all subsequent predictions, response curves, and suitability mapping. Second, a separate cross-validation procedure was carried out to obtain an honest estimate of how well the model generalises to unseen regions.

Standard random cross-validation tends to produce optimistic performance estimates for spatial data, because nearby training and test points share similar environmental conditions through spatial autocorrelation \citep{Roberts2017}. To avoid this bias, we used 4-fold spatially blocked cross-validation via the \texttt{GeographicKFold} class in \texttt{elapid} \citep{Valavi2019}. The splitter partitions all 18,310 sample locations into four spatially contiguous blocks using $k$-means clustering on point coordinates, so that geographically proximate points are always assigned to the same block. In each of the four iterations, three blocks serve as training data and the remaining block is held out for testing. Critically, a \emph{new, independent} MaxEnt model (with identical hyperparameters) is fitted from scratch on the training blocks alone and then evaluated on the held-out block. This ensures the test predictions are made on a geographic region the model has never seen during fitting. The procedure is repeated four times, rotating which block is held out, yielding four independent test-AUC values.

Model discrimination was evaluated using the area under the receiver operating characteristic (ROC) curve, or AUC. The ROC curve is constructed by sweeping a decision threshold $\tau$ across the full range of predicted suitability scores. At each value of $\tau$, every sample with a predicted suitability above $\tau$ is classified as ``present'' and every sample below $\tau$ as ``background''. This yields a true positive rate (TPR: the fraction of actual presence sites correctly classified) and a false positive rate (FPR: the fraction of background points incorrectly classified as present). Plotting TPR against FPR across all thresholds from 0 to 1 traces the ROC curve. The AUC is the area under this curve, computed by numerical integration. It is equivalent to the probability that a randomly chosen presence site is assigned a higher predicted suitability than a randomly chosen background point. An AUC of 0.5 indicates discrimination no better than random (the ROC curve follows the diagonal), while 1.0 indicates perfect separation. In the presence--background setting used here, the FPR refers to background points ranked above a given threshold rather than confirmed absences, so AUC measures the model's ability to distinguish occupied environments from the landscape as a whole \citep{Phillips2006, Elith2011}. Values above ${\sim}$0.7 are generally considered useful for presence--background models \citep{Elith2006}. The ROC curves for each cross-validation fold and the full training dataset are shown in Fig.~\ref{fig:roc_curves}.
...
The full-data MaxEnt model achieved a training AUC of 0.80. Under spatially blocked cross-validation, the four held-out folds yielded AUC values of 0.65, 0.68, 0.74, and 0.69 (Fig.~\ref{fig:roc_curves}), giving a mean of 0.69 ($\pm$ 0.03). The gap between training and cross-validated AUC (0.80 vs 0.69) quantifies the degree to which spatial autocorrelation inflates apparent performance: the model fits well within the regions it has seen, but transfers less perfectly to geographically distinct areas \citep{Roberts2017}. A spatially cross-validated AUC of 0.69 nonetheless indicates moderate but genuine discriminatory ability, particularly given that the model relies on only four satellite-derived predictors across a large and environmentally heterogeneous study area.
